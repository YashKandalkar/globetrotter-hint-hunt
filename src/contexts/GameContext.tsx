...
import { getRandomDestination, getMultipleDestinations, checkAnswer, updateUserScore } from '../lib/supabase';
...
export function GameProvider({ children }: { children: ReactNode }) {
  ...
  const loadNewGame = async () => {
    try {
      ...
      allOptions.splice(correctAnswerIndex, 0, destination);
  ...
  const submitAnswer = async (answer: string) => {
    try {
      ...
      if (user) {
        await updateUserScore(user.id, result.correct);
      }
